...
PM2_START_CMD="pm2 start ./dist/app/server.js --name $PM2_APP_NAME --time -f --update-env"
PM2_RESTART_CMD="pm2 restart $PM2_APP_NAME --time -f --update-env || $PM2_START_CMD"
...
rsync -avz --delete --exclude 'node_modules' --exclude 'dist' --exclude 'coverage' --exclude 'package-lock.json' --exclude '*.md' --exclude '.git' --exclude 'data' --exclude 'prisma/migrations' --exclude 'prisma/dev.db' --exclude 'prisma/dev.db-journal' -e "ssh $SSH_OPTS" ./ "$REMOTE_USER@$REMOTE_HOST:$REMOTE_APP_DIR/"
