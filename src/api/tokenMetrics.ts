...
export class TokenMetricsClient {
  ...
  async getTraderGrades(symbols?: string[] | null): Promise<TokenMetricsTraderGrade[]> {
    ...
    if (!this.client) {
      console.error("getTraderGrades called, but the underlying TokenMetrics SDK client is not initialized. Ensure mocking is intended or the API key is valid.");
...
export function createTokenMetricsClient(): TokenMetricsClient {
  const useMock = process.env.USE_MOCK_TRADER_GRADES === 'true';

  if (useMock) {
    console.log('Mocking enabled: Creating mock TokenMetricsClient.');
    // Create an instance WITHOUT providing an API key to the constructor
    const mockClient = new TokenMetricsClient(); // this.client inside will be undefined

    // Define and assign the mock method directly to this instance
    mockClient.getTraderGrades = async (/* symbols?: string[] | null */): Promise<TokenMetricsTraderGrade[]> => {
      console.log('Using mock trader grades data (assigned mock method)...');
      let mockFilePath: string;

      // Determine path based on environment
      if (process.env.NODE_ENV === 'production') {
          // In container (NODE_ENV=production), path is absolute from /app root
          mockFilePath = '/app/static_data/trader_grades_response.json';
      } else {
          // Local development, path is relative to project root where command is run
          mockFilePath = path.join(process.cwd(), 'static_data/trader_grades_response.json');
      }
      console.log(`Attempting to read mock file from: ${mockFilePath}`); // Log the path being used

      try {
        const mockDataRaw = await fs.readFile(mockFilePath, 'utf-8');
        const mockResponse = JSON.parse(mockDataRaw);
        // Use mockResponse directly if it's the array, or mockResponse.data if nested
        ...
        // Check if the error is ENOENT (file not found) and provide a more specific message
        if (error instanceof Error && 'code' in error && error.code === 'ENOENT') {
             console.error(`Mock file not found at calculated path. Check if 'data/trader_grades_response.json' exists relative to project root.`);
        }
        return [];
      }
    };
    ...
    // Mocking is disabled, create the real client
    ...
    if (!apiKey) { // Stricter check: Key must exist
