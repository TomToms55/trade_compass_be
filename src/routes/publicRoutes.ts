...
type StorageService = ReturnType<typeof import('../services/storage').default>;
...
export default async function publicRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
    ...
    if (!storageService) {
        throw new Error('Storage service not decorated on Fastify instance');
    }
    ...
    fastify.post('/register', { schema: registerSchema }, async (request, reply) => {
        try {
            const { apiKey, apiSecret, password } = request.body as userService.UserRegistrationData;
